...
def add_noise_and_round(data, noise_level=0.05):
...
def generate_pattern(pattern_type):
    ...
    return add_noise_and_round(pattern, noise_level=0.05)

# Generate initial microservices data
patterns = ['stable'] * 2 + ['work_hours'] * 4 + ['night'] * 2 + ['evening'] * 2 + ['random'] * 2
microservices = np.array([generate_pattern(p) for p in patterns])

# Generate symmetrical patterns with little noise
symmetrical = add_noise_and_round(100 - microservices, noise_level=0.02)

# Combine original patterns and symmetrical patterns
all_microservices = np.vstack((microservices, symmetrical))
